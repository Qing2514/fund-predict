...
model = Sequential()
model.add(LSTM(units=128, activation='relu', input_shape=(features_set.shape[1], 1), return_sequences=True))
...
history = model.fit(features_set, labels, epochs=100, batch_size=30, validation_split=0.2, verbose=1, shuffle=True)
score = model.evaluate(features_set, labels, batch_size=128)

model.save('model/model_' + fundCode + '_test_3.h5')
